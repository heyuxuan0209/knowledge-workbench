-- KW 解读.app（ADR-067）：X/YouTube 原生 Mac App 里插件进不去，走「拷贝链接 → 点我」通路。
-- 读剪贴板链接 → 打开 KW 工作台 ?analyze=<链接> 自动解读（前端 WorkbenchPage 已支持该参数）。
-- 重新编译：osacompile -o ~/Applications/"KW 解读.app" extension/mac-launcher/kw-jiedu.applescript
...
if u starts with "http" then
	...
	display notification "先在 X / YouTube 里拷贝链接，再点我" with title "KW 解读" sound name "Funk"
end if
