...
export function upsertContents(items) {
  ...
  for (const { content, sourceInfo } of items) {
    try {
      // sourceInfo.sourceId：调用方已经知道对应的 source（如 RSS 同步器读 source_platforms
      // 时已带出 source_id），跳过按 platform+handle 判重的查找，避免重复查询。
      const sourceId = sourceInfo?.sourceId ?? findOrCreateSource(db, sourceInfo);
