...
export async function batchSummarize(items) {
  ...
  const prompt = `为以下每条内容写一段中文概括（120-180 字，2-4 句）：先说清**讲了什么 / 核心结论**，再点出**关键信息或值得注意处**，让人不点进去也知道主要内容。
有原文摘录的基于摘录写实、可含具体数字/名称；只有标题的基于标题保守概括、宁短勿编（不要编造细节）。
不要写"本文""这篇文章"之类的套话，直接讲内容。
