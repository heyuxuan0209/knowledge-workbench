  ...
  try {
    ...
    const pretransformed = limitedItems.map(item => {
      ...
      try {
        ...
        // 近 30 天窗口：砍未登记杂源 + T1 官方大源的陈旧存量；只有 T2 及以下的登记优质源
        // （个人 builder，更新慢也留、只按 AI 相关性过滤）豁免。无 pubDate 解析不出来的保留，宁可留。
        const exemptFreshness = src?.registered && src?.tier !== 'T1';
        const pub = new Date(t.content.published_at).getTime();
        if (pub && pub < Date.now() - 30 * 864e5 && !exemptFreshness) return null;
