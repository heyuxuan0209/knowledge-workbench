...
// 导航点击事件
document.getElementById('navSources')?.addEventListener('click', (e) => {
    e.preventDefault();
    switchView('sources');
    loadSources();
...
async function loadSources() {
    try {
        const res = await fetch(`${API_BASE}/api/sources?track_mode=active`);
        const json = await res.json();

        if (!json.success) throw new Error(json.error || 'Failed to load sources');

        const sourcesList = document.getElementById('sourcesList');
        const sourcesCount = document.getElementById('sourcesCount');

        sourcesCount.textContent = json.data.length;

        if (json.data.length === 0) {
            sourcesList.innerHTML = '<div class="empty-state">尚未添加信息源</div>';
            return;
        }

        sourcesList.innerHTML = json.data.map(source => `
            <div class="source-card" data-source-id="${source.id}">
                <div class="source-card-header">
                    <div class="source-card-info">
                        <div class="source-card-platform">${source.platform}</div>
                        <div class="source-card-name">${source.display_name}</div>
                        <div class="source-card-url">${source.handle || source.url || ''}</div>
                    </div>
                </div>
                <div class="source-card-meta">
                    <span class="source-card-status active">追踪中</span>
                    <span>·</span>
                    <span>${source.created_at ? timeAgo(source.created_at) : '未知'}</span>
                </div>
                <div class="source-card-actions">
                    <button class="btn-sync" data-source-id="${source.id}">立即同步</button>
                    <button class="btn-delete" data-source-id="${source.id}">删除</button>
                </div>
            </div>
        `).join('');

        // 绑定删除事件
        sourcesList.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const sourceId = e.target.dataset.sourceId;
                if (!confirm('确定删除这个信息源吗？')) return;

                try {
                    const res = await fetch(`${API_BASE}/api/sources/${sourceId}`, { method: 'DELETE' });
                    const json = await res.json();
                    if (json.success) {
                        loadSources();
                    } else {
        ...
        document.getElementById('sourcesList').innerHTML = '<div class="empty-state">加载失败，请刷新重试</div>';
    }
}
...
document.getElementById('addSourceBtn')?.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/sources/add-by-input`, {
            method: 'POST',
        ...
        if (!json.success) {
            alert('添加失败: ' + (json.error || '无法识别此链接'));
            return;
        }

        input.value = '';
        loadSources();
        alert(`成功添加: ${json.data.platform} - ${json.data.display_name}`);
    } catch (error) {
...
function renderCard(item) {
    ...
    const titleContent = item.url
        ? `<a href="${item.url}" target="_blank" rel="noopener noreferrer" class="card-title-link">${item.zh_title}</a>`
        : item.zh_title;
    ...
    <div class="feed-card" data-id="${item.id}">
        ...
        <h3 class="card-title">${titleContent}</h3>
        ...
        <div class="card-meta">
            ...
            ${item.url ? `<span>·</span><a href="${item.url}" target="_blank" rel="noopener noreferrer" class="card-source-link">查看原文 ↗</a>` : ''}
        </div>
