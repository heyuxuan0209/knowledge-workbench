...
**里程碑**：采集 → 理解 → 沉淀 → 主题 → **灵感** → 创作 → 洞察 已全部产品化，并在其上加了**本地语义层**（语义搜索 / RAG / 查重 / 关联，零 API 成本）与**创作台 v2**（文体 × 平台正交拼装）。

> **下一步（尚未产品化）**：把「发布 → 数据 → 复盘 → 自进化」这后半环做出来——目前它仍是手动 / 愿景。这半环也是本项目北极星（内容是否让读者更有收获）真正的落地抓手。
...
- **即时分析（万能收口）**：任意链接 / 大段文字 / 音频 / PDF 丢进来 → 抓取或本地转写 + 结构化中文精读（文章 / 视频字幕 / 播客转写）；**特殊来源智能处理**（AI HOT 收录页·推文先查本地库不空抓、公众号直抽正文）
...
- 从主题页 / 灵感 / 素材起稿；段落级溯源（[素材N] 标记落库，导出时转参考来源列表）
