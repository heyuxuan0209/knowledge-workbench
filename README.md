...
- **探讨**：主题页右栏与 AI 对话，综述 + 已并入素材做弹药（引证据、指矛盾、提反例）
- changelog 时间线 = 认知演进史

### ✍️ 创作 —— 内容生产
- 主题页一键起稿，**平台模板即文件**：`reference/prompts/creation/platforms/` 加一个 md 就多一个平台（内置长文 / thread / 口播脚本 / 小红书图文）
- **平台裂变**：写好的稿子点其他平台按钮，改写为该平台规格版（新稿另存，原稿不动）
- 段落级溯源（[素材N] 标记落库，导出时转参考来源列表）；右栏可插入素材按**与当前草稿的相关度**排序（本地 TF 余弦）
- 修改工具包：**审稿**（挑剔读者/结构编辑/事实核查三视角批注，逐条可应用可撤销）、**3 个改法**（选段给锋利/具体/简洁三个候选）、去 AI 味审校、标题候选、指令改写
