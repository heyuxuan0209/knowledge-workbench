...
- **数据库备份**：VPS 每日自动部署前用 SQLite `VACUUM INTO` 生成一致性备份，最多保留30份；Mac launchd `com.knowledge-workbench.backup` 每天 07:20 拉取最新一份到 `~/Backups/knowledge-workbench/`，校验 SQLite 完整性与 SHA-256 后以只读 `0400` 原子落盘，同样最多保留30份。Mac 任务失败会发系统通知。Mac 副本从不放进 `backend/data/`，也不参与任何服务运行。launchd 运行副本安装在 `~/Library/Application Support/KnowledgeWorkbench/`，避免 macOS TCC 禁止后台任务访问 `Documents`；改源码后须同步复制运行副本。
