...
- **数据库备份**：VPS 每日自动部署前用 SQLite `VACUUM INTO` 生成一致性备份，最多保留30份；Mac launchd `com.knowledge-workbench.backup` 每天 07:20 拉取最新一份到 `~/Backups/knowledge-workbench/`，校验 SQLite 完整性与 SHA-256 后原子落盘，同样最多保留30份。Mac 任务失败会发系统通知。
