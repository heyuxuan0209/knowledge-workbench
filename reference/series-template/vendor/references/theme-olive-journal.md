...
⚠️ `{{强调词}}` 硬规则（2026-08-03 实测踩坑）：强调词是主标题之外的补充词（如「现场手记」「实测」「复盘」「亲历」），绝不能是主标题里已经出现过的字词——实测 LLM 会把主标题的后半截再填一遍，产出「10 倍的个体，1.2 倍的组织 · 1.2 倍的组织」这种重复。想不出合适的补充词时，把 `<span style="color:#4d4f46;"><span leaf="">&nbsp;·&nbsp;</span></span>` 连同强调词那个 `<span>` 整段删掉，只保留主标题。
