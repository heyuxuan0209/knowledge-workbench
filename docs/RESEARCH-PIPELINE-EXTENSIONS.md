...
| 条目 | 落点 | 做法 | 状态 |
|------|------|------|------|
| Jina Reader 兜底 | content-ingestion.js | readability 失败（SPA/反爬/公众号单篇）→ 重试 `https://r.jina.ai/<url>`，只读合规 | ✅ 已落地（2026-07-15，ADR-013） |
| 结构化解读模板升级 | ephemeral chat prompt / 解读产物 | 吸收 link2article 模板：TL;DR / 核心观点 / 金句（带时间戳）/ **可复用素材（选题角度·数据案例·争议钩子）** / **局限与存疑**（契合诚实原则）；同时收掉 M2 遗留的"结构化解读升级" | ✅ 已落地（2026-07-15，ADR-013） |
